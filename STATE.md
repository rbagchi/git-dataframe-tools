...
- `_parse_file_stat_line` in `src/git2df/git_parser.py` has been refined to handle both 3-part and 4-part file stat lines, making it more robust.
- Unit tests for `git_parser.py` have been expanded with new golden files to validate the parsing of file changes, including additions, deletions, and `change_type`.

**Status:** Completed. All tests are passing.
