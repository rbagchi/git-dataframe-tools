...
def _display_author_specific_stats(config: GitAnalysisConfig, author_stats: List[Dict[str, Any]]) -> int:
    if config.use_current_user:
        logger.info(
            f"Looking up stats for current user: {config.current_user_name} <{config.current_user_email}>"
        )

    author_matches = [a for a in author_stats if config.author_query.lower() in a['author_name'].lower() or config.author_query.lower() in a['author_email'].lower()]
...
def _display_full_ranking(config: GitAnalysisConfig, author_list: List[Dict[str, Any]]) -> int:
    print()
