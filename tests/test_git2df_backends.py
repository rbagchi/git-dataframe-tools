...
def test_git_cli_backend_get_raw_log_output_with_since_until(mock_subprocess_run):
    ...
    output = backend.get_raw_log_output(
        repo_path, log_args=log_args, since=since_arg, until=until_arg
    )
...
@patch("git2df.git_cli_utils.subprocess.run")
def test_git_cli_backend_get_raw_log_output_with_all_filters_and_paths(
    ...
    mock_subprocess_run.return_value.strip.return_value = (
        "all filters and paths mocked output"
    )
