...
from git2df.git_parser import _parse_commit_metadata_line, _parse_file_stat_line, FileChange, parse_git_log
...
def test_parse_file_stat_line_valid_modified():
    ...
    expected = FileChange(file_path="file.txt", additions=10, deletions=5, change_type="M")
    result = _parse_file_stat_line(line)
    assert result == expected

def test_parse_file_stat_line_valid_no_explicit_type():
    line = "10\t5\tfile.txt"
    expected = FileChange(file_path="file.txt", additions=10, deletions=5, change_type="M") # Defaults to M
    result = _parse_file_stat_line(line)
    assert result == expected

def test_parse_file_stat_line_valid_added():
    line = "10\t0\tA\tnew_file.txt"
    expected = FileChange(file_path="new_file.txt", additions=10, deletions=0, change_type="A")
    result = _parse_file_stat_line(line)
    assert result == expected

def test_parse_file_stat_line_valid_deleted():
    line = "0\t5\tD\tdeleted_file.txt"
    expected = FileChange(file_path="deleted_file.txt", additions=0, deletions=5, change_type="D")
    result = _parse_file_stat_line(line)
    assert result == expected

def test_parse_file_stat_line_valid_dash_stats():
    line = "-\t-\tfile_with_no_stats.txt"
    expected = FileChange(file_path="file_with_no_stats.txt", additions=0, deletions=0, change_type="M")
    result = _parse_file_stat_line(line)
    assert result == expected
...
def test_parse_file_stat_line_malformed_extra_fields():
    ...
    expected = FileChange(file_path="file.txt\textra", additions=10, deletions=5, change_type="M")
    result = _parse_file_stat_line(line)
    assert result == expected
